...
def create_start(dag):
    return DummyOperator(task_id="start", dag=dag)


def create_finish(dag):
    return DummyOperator(task_id="finish", dag=dag)


def create_spark_submit(dag, config):
    return SparkSubmitOperator(
        task_id=f'spark_submit_{config["name"]}',
        application=config["application"],
        name=config["name"],
        conn_id=config["conn"],
        application_args=[config["name"], config["spark_master"]],
...
def create_pipeline(dag_id, configs):
    ...
    spark_submit_trips = create_spark_submit(dag, configs[0])
    configs = configs[1:]
    for config in configs:
        spark_submit = create_spark_submit(dag, config)
        (start >> spark_submit >> spark_submit_trips >> finish)
